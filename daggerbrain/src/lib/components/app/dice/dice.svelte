<script lang="ts">
	...
	let {
		class: className = '',
		onRollEnd = () => {}
	}: { class?: string; onRollEnd?: (result: Notation, total: number) => void } = $props();
	...
	let diceBox: HTMLDivElement = $state.raw(null!);
	...
	function before_roll(vectors: any, notation: any, callback: (result: any) => void) {
	...
	function after_roll(notation: any, result: any) {
		const total = result.reduce((acc: number, curr: number) => acc + curr, 0);
		onRollEnd(notation as Notation, total);
	}

	function roll_dice(inputString: string) {
		showDice = true;
		box.rolling = false;
		box.start_throw(
			//@ts-ignore
			() => window.teal.dice.parse_notation(inputString),
			before_roll,
			after_roll
		);
	}
	...
	onMount(async () => {
		...
		//@ts-ignore
		if (window.teal === undefined || window.teal.dice === undefined) {
			console.warn('Dice is not loaded, waiting for 2 seconds');
			await new Promise((resolve) => setTimeout(resolve, 2000));
		}

		//@ts-ignore
		if (window.teal === undefined || window.teal.dice === undefined) {
			console.error('Failed to create dice box');
		}
		loaded = true;
...
<svelte:head>
	<script src="/teal/three.min.js"></script>
	<script src="/teal/cannon.min.js"></script>
	<script src="/teal/webgl.js"></script>
	<script src="/teal/teal.js"></script>
	<script src="/teal/dice.js"></script>
</svelte:head>

{#if loaded}
	<div transition:fade={{ duration: 200 }} class={cn('fixed flex flex-col gap-2', className)}>
		{#if expanded}
			<div
				transition:slide={{ duration: 100 }}
				class={cn(
					'flex w-min flex-col items-center gap-1 rounded-full border-2 border-b-0 bg-primary-muted px-2 py-4 text-foreground  shadow-lg'
				)}
			>
				<button
					class="relative size-12 rounded-md p-2 hover:bg-primary/20"
					onclick={() => {
						counts.d4++;
					}}
					oncontextmenu={(e) => {
						e.preventDefault();
						counts.d4 = Math.max(counts.d4 - 1, 0);
					}}
				>
					{#if counts.d4 > 0}
						<span
							class="absolute top-1/2 left-full -translate-x-4 -translate-y-1/2 rounded-full bg-muted px-2 text-xs font-medium"
							>{counts.d4}</span
						>
					{/if}
					<svg
				...
				<button
					...
					oncontextmenu={(e) => {
						e.preventDefault();
						counts.d6 = Math.max(counts.d6 - 1, 0);
					}}
				>
					{#if counts.d6 > 0}
						<span
				...
				<button
					...
					oncontextmenu={(e) => {
						e.preventDefault();
						counts.d8 = Math.max(counts.d8 - 1, 0);
					}}
				>
					{#if counts.d8 > 0}
						<span
					...
					>
						<defs>
							<path
								id="p3"
								d="m125.67 128.55c6.67 6.67 12.56 14.1 19.26 20.76-1.97 3.66-4.96 5.87-7.56 9.1-3.7 4.49-7.76 8.66-11.58 13.06-6.62-6.92-13.1-13.99-19.74-20.89 6.41-7.44 12.97-14.81 19.62-22.03zm-16.87 21.1c8.33-.03 16.67.03 25-.24-2.53-6.23-5.6-12.21-8.19-18.42-5.82 6.02-11.23 12.43-16.81 18.66zm.07 1.54c5.59 5.95 11.2 11.89 16.82 17.8 2.78-5.97 5.45-11.99 8.26-17.95-8.36-.05-16.72.09-25.08.15z"
							/>
							<path
								id="p8"
								d="m125.61 130.99c2.59 6.21 5.66 12.19 8.19 18.42-8.33.27-16.67.21-25 .24 5.58-6.23 10.99-12.64 16.81-18.66z"
							/>
							<path
								id="p22"
								d="m108.87 151.19c8.36-.06 16.72-.2 25.08-.15-2.81 5.96-5.48 11.98-8.26 17.95-5.62-5.91-11.23-11.85-16.82-17.8z"
							/>
						</defs>
						<use
							transform="matrix(.35278 0 0 .35278 -35.856 -44.354)"
							width="100%"
							height="100%"
							fill="currentColor"
							xlink:href="#p3"
				...
				<button
					...
					oncontextmenu={(e) => {
						e.preventDefault();
						counts.d10 = Math.max(counts.d10 - 1, 0);
					}}
				>
					{#if counts.d10 > 0}
						<span
					...
					>
						<defs>
							<path
								id="p4"
								d="m174.87 129.15c6.22 5.43 12.63 10.68 18.74 16.25 1.61 1.28 1.62 3.61 2.13 5.5-6.72 6.57-13.28 13.3-19.99 19.88-7.32-6.03-14.12-12.73-21.16-19.09.55-2.53.28-4.83 2.42-6.63 5.93-5.3 11.74-10.83 17.86-15.91zm-8.37 15.22c2.95 1.05 5.85 2.43 8.89 3.21 3.58-.4 7.16-1.88 10.71-2.63-3.49-4.69-7.45-9.03-11.2-13.5-2.95 4.2-5.69 8.55-8.4 12.92zm-10.17 5.51c2.68-1.83 5.53-3.52 8.03-5.6 2.41-2.55 4.06-5.93 5.8-8.96-3.83 3.45-7.71 6.84-11.51 10.32-1.38 1.16-1.8 2.56-2.32 4.24zm.47 1.67c5.9 5.64 12.07 10.99 18.01 16.59.02-6.34 0-12.67-.01-19.01-3.02-1.06-5.98-2.3-8.94-3.52-3.17 1.65-6.1 3.87-9.06 5.94z"
							/>
							<path
								id="p10"
								d="m174.9 131.45c3.75 4.47 7.71 8.81 11.2 13.5-3.55.75-7.13 2.23-10.71 2.63-3.04-.78-5.94-2.16-8.89-3.21 2.71-4.37 5.45-8.72 8.4-12.92z"
							/>
							<path
								id="p14"
								d="m170.16 135.32c-1.74 3.03-3.39 6.41-5.8 8.96-2.5 2.08-5.35 3.77-8.03 5.6.52-1.68.94-3.08 2.32-4.24 3.8-3.48 7.68-6.87 11.51-10.32z"
							/>
							<path
								id="p20"
								d="m165.86 145.61c2.96 1.22 5.92 2.46 8.94 3.52.01 6.34.03 12.67.01 19.01-5.94-5.6-12.11-10.95-18.01-16.59 2.96-2.07 5.89-4.29 9.06-5.94z"
							/>
						</defs>
						<use
							transform="matrix(.35278 0 0 .35278 -53.339 -44.38)"
							width="100%"
							height="100%"
							fill="currentColor"
							xlink:href="#p4"
				...
				<button
					...
					oncontextmenu={(e) => {
						e.preventDefault();
						counts.d12 = Math.max(counts.d12 - 1, 0);
					}}
				>
					{#if counts.d12 > 0}
						<span
					...
					>
						...
						<use
							transform="matrix(.35278 0 0 .35278 -70.876 -44.38)"
							width="100%"
							height="100%"
							fill="currentColor"
							xlink:href="#p2"
				...
				<button
					...
					oncontextmenu={(e) => {
						e.preventDefault();
						counts.d20 = Math.max(counts.d20 - 1, 0);
					}}
				>
					{#if counts.d20 > 0}
						<span
					...
					>
						<defs>
							...
							<path
								id="p5"
								d="m274.64 128.92c3.25 3.81 6.72 7.43 9.91 11.3-6.72.03-13.44.01-20.15-.03 3.33-3.8 6.52-7.83 10.24-11.27z"
							/>
							<path
								id="p11"
								d="m269.41 131.73c-2.01 2.51-3.94 5.23-6.28 7.44-1.22 1.54-3.61 0-5.2-.09 2.64-1.74 5.33-3.39 7.99-5.11 1.16-.75 2.33-1.49 3.49-2.24z"
							/>
							<path
								id="p17"
								d="m256.82 140.36c1.55.42 3.1.83 4.66 1.23-1.88 4.46-3.6 8.97-5.57 13.39.06-3.81.28-7.6.61-11.39.1-1.08.2-2.16.3-3.23z"
							/>
							<path
								id="p18"
								d="m263.85 141.68c7.07.15 14.15.1 21.22.02-3.52 6.11-7.04 12.22-10.53 18.34-3.71-6.04-7.14-12.23-10.69-18.36z"
							/>
							<path
								id="p19"
								d="m262.65 142.84c3.52 5.91 7.18 11.78 10.28 17.92-5.68-.5-11.38-.81-17.07-1.16 2.29-5.58 4.39-11.23 6.79-16.76z"
							/>
							<path
								id="p23"
								d="m257.68 161.27c5.35.17 10.68.59 16.01 1.03.04 2.87.03 5.74-.04 8.61-5.36-3.16-10.56-6.57-15.97-9.64z"
							/>
						</defs>
						<use
							transform="matrix(.35278 0 0 .35278 -88.3 -44.431)"
							width="100%"
							height="100%"
							fill="currentColor"
							xlink:href="#p1"
				...
				>
					{#if counts.duality > 0}
						<span
							class="absolute top-1/2 left-full z-40 -translate-x-4 -translate-y-1/2 rounded-full bg-muted px-2 text-xs font-medium"
							>{counts.duality}</span
						>
					{/if}
					<svg
						class="absolute top-1.5 left-1 size-7 text-background"
						width="64"
						height="64"
						version="1.1"
						viewBox="0 0 16.933 16.933"
					...
					<svg
						class="absolute right-1 bottom-2 size-7 text-background"
						width="64"
						height="64"
						version="1.1"
						viewBox="0 0 16.933 16.933"
		...
		<div
			class="flex shrink-0 items-center justify-center rounded-full border-2 bg-primary-muted px-2 py-2 shadow-lg"
		>
			{#if expanded}
				{#if counts.d4 > 0 || counts.d6 > 0 || counts.d8 > 0 || counts.d10 > 0 || counts.d12 > 0 || counts.d20 > 0 || counts.duality > 0}
					<button
						onclick={close}
						class="relative grid size-12 place-items-center rounded-l-full border-2 border-accent/10 bg-muted/50 text-accent hover:bg-accent/20"
					>
						<X class="size-6 stroke-1" />
					</button>
					<button
						onclick={() => {
							roll_dice(rollString);
							close();
						}}
						class="h-12 w-16 rounded-r-full border border-l-0 border-accent/10 bg-accent/10 pl-1 text-sm text-accent hover:bg-accent/20"
					>
						Roll
					</button>
				{:else}
					<button
						onclick={close}
						class="relative grid size-12 place-items-center rounded-full border-2 border-accent/10 bg-muted/50 text-accent hover:bg-accent/20"
					>
						<X class="size-6 stroke-1" />
					</button>
				{/if}
...
<div bind:this={diceBox} id="dice-box" class="pointer-events-none fixed inset-0 z-45"></div>
{#if showDice}
	<!-- svelte-ignore a11y_consider_explicit_label -->
	<button
