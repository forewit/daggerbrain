...
<div
	class={cn(
		'flex flex-col items-center border-y bg-accent/5 bg-primary/15 p-6 px-2 pt-4 shadow-xl  sm:mx-auto sm:rounded-2xl sm:px-6',
		className
	)}
>
	<div class="mb-4 flex items-center justify-center gap-4">
		<p class="font- text-center text-lg font-semibold">Characters</p>
		<label
			class={cn(
				buttonVariants({ variant: 'outline', size: 'sm' }),
				'cursor-pointer gap-3 rounded-full border-primary/20 bg-primary/5 px-4 hover:bg-primary/5',
				...
				{#each characterList as char}
					...
					<div class="mx-auto w-full overflow-hidden rounded-lg border bg-background p-2 shadow-lg">
							...
							<div class="grow truncate pt-0.5">
								<div class="mr-2 flex items-center justify-between">
									...
									{#if char.active_conditions.length > 0}
										<div class="flex items-center gap-1">
											{#each char.active_conditions as id}
												{#if id === 'hidden'}
													<svg
														class="size-4"
														viewBox="0 0 512 512"
								...
								<div class="flex items-center gap-4 truncate text-xs text-muted-foreground">
									...
									<p class="flex items-center gap-1">
										...
										{char.marked_hp} / {char.derived_character_summary?.max_hp ?? 0}
									</p>
									<p class="flex items-center gap-1">
										<Shield class="size-3.5" />
										{char.marked_armor} / {char.derived_character_summary?.max_armor ?? 0}
									</p>
									<p class="flex items-center gap-1">
										<Zap class="size-3.5" />
										{char.marked_stress} / {char.derived_character_summary?.max_stress ?? 6}
									</p>
								</div>
						...
						<div class="flex gap-2 bg-background pt-2">
							<Button variant="outline" size="sm" class="grow" href={`/characters/${char.id}/`}>
								View
							</Button>
							{#if canEdit}
								<Button
