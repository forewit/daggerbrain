...
<div class={cn('flex w-full max-w-[400px] flex-col rounded-xl bg-background shadow-lg', className)}>
			...
			<div class="flex min-w-0 gap-3">
				...
				<div class="flex min-w-0 grow flex-col gap-1">
					...
					<div class="flex items-end">
						...
						<div
							title={character.active_conditions.join(', ') || 'No conditions'}
							class="flex grow flex-col items-center justify-center gap-0.5"
						>
							<p class="text-xs font-bold text-muted-foreground">Conditions</p>
	...
	<div class="flex gap-2 p-2">
		<Button variant="outline" size="sm" class="grow" href={`/characters/${character.id}/`}>
			View
		</Button>
		{#if canEdit}
			<Button variant="outline" class="grow" size="sm" href={`/characters/${character.id}/edit`}>
				Edit
			</Button>
		{/if}
	</div>
