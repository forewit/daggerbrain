<script lang="ts">
	...
	let hasChanges = $derived.by(() => {
		...
		const featuresMatch =
			JSON.stringify(formFeatures) === JSON.stringify(armor.features);
	...
	$effect(() => {
		if (armor) {
			...
			formMajorThreshold = armor.damage_thresholds.major === 0 ? '' : String(armor.damage_thresholds.major);
			formSevereThreshold = armor.damage_thresholds.severe === 0 ? '' : String(armor.damage_thresholds.severe);
	...
	function handleSave() {
		...
		// Update the armor prop with form values
		armor = {
			...armor,
...
<div class="flex flex-col gap-4">
	...
	<div class="flex flex-col gap-1">
		<label for="hb-armor-title" class="text-xs font-medium text-muted-foreground">Name</label>
		<Input id="hb-armor-title" bind:value={formTitle} placeholder="Armor name" />
	</div>
