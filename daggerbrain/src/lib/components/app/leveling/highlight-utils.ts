...
export function calculate_highlighted(
	...
	for (const tier_prefix of tiers_to_check) {
		...
		tier_checks.push(
			(choices.A.option_id === experience_bonus_option &&
				choices.A.selected_experiences.length < 2) ||
				(choices.B.option_id === experience_bonus_option &&
					choices.B.selected_experiences.length < 2)
		);
	...
	return (
		...
		character.level_up_domain_card_ids[level as keyof typeof character.level_up_domain_card_ids]
			.A === null ||
