<script lang="ts">
	...
	let restrainedOpen = $state(false);
	let vulnerableOpen = $state(false);
					...
					<div class="flex items-center gap-2">
						<Switch
							...
							onCheckedChange={(checked) => {
								if (checked) {
									context.addCondition('hidden');
								} else {
									context.removeCondition('hidden');
								}
							}}
			...
			<Collapsible.Root bind:open={restrainedOpen}>
				...
				>
					<div class="flex items-center gap-2">
						...
						<span class="text-sm font-medium">Restrained</span>
					</div>
					<div class="flex items-center gap-2">
						<Switch
							...
							onCheckedChange={(checked) => {
								if (checked) {
									context.addCondition('restrained');
								} else {
									context.removeCondition('restrained');
								}
							}}
						...
						/>
						<ChevronLeft
							class={cn('size-4 transition-transform', restrainedOpen && '-rotate-90')}
						/>
					</div>
				</Collapsible.Trigger>
				<Collapsible.Content>
			...
			<Collapsible.Root bind:open={vulnerableOpen}>
				...
				>
					<div class="flex items-center gap-2">
						...
						<span class="text-sm font-medium">Vulnerable</span>
					</div>
					<div class="flex items-center gap-2">
						<Switch
							...
							onCheckedChange={(checked) => {
								if (checked) {
									context.addCondition('vulnerable');
								} else {
									context.removeCondition('vulnerable');
								}
							}}
						...
						/>
						<ChevronLeft
							class={cn('size-4 transition-transform', vulnerableOpen && '-rotate-90')}
						/>
					</div>
				</Collapsible.Trigger>
				<Collapsible.Content>
