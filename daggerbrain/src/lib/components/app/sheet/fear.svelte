...
<div class={cn('w-min ', className)}>
	<div class="flex flex-col items-center gap-x-4 gap-y-2 sm:flex-row">
		<button
			...
			class="font-eveleth leading-none font-bold text-destructive">FEAR</button
		>

		<div class="relative mx-auto w-[370px]">
			<svg
				class="w-full text-destructive/50"
			...
			<div class="absolute top-1/2 left-8.5 flex -translate-y-1/2 items-center gap-5">
				<div class="flex items-center gap-2.5">
					{#each Array(MAX_FEAR) as _, index}
						...
						<button
							...
							type="button"
							class={cn(
								'flex size-[16px] items-center justify-center rounded-full border-2 border-destructive transition-colors',
								!isGM && 'cursor-pointer opacity-50',
								isGM && 'hover:border-destructive/80'
							)}
						>
							{#if fearLevel <= fearValue}
								<div class="size-[8px] rounded-full bg-destructive"></div>
							{/if}
						</button>
					{/each}
				</div>
	...
	<div class="mt-4 flex items-center justify-center gap-4">
		<Button onclick={() => {
			fearValue = Math.max(0, fearValue + 1);
			onUpdate(fearValue);
		}} variant="outline" size="sm" class="w-min">Add Fear</Button>
		<Button onclick={() => {
			fearValue = Math.max(0, fearValue - 1);
			onUpdate(fearValue);
		}} variant="outline" size="sm" class="w-min">Spend Fear</Button>
