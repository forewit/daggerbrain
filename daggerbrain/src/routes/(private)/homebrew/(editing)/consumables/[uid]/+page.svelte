<script lang="ts">
	...
	// Get consumable directly from homebrew context - auto-save is handled by the context
	...
	$effect(() => {
		if (consumable) {
			editingHeader.set(consumable.title, 'Editing Consumable');
		}
	});
</script>
				...
				<div class="flex flex-col gap-6 lg:flex-row lg:items-start">
					...
					<div class="flex-1 rounded-lg border bg-card p-6">
						...
						<div class="flex flex-col gap-6">
							...
							<table class="w-full border-collapse text-sm">
								<tbody>
									<tr>
										<th class="py-2 pr-4 text-left font-normal text-muted-foreground">Rarity Roll</th>
										<td class="py-2 text-right">{consumable.rarity_roll}</td>
									</tr>
								</tbody>
							</table>
						</div>
					...
					<div class="w-full lg:w-auto lg:min-w-[300px] lg:max-w-[300px]">
						<div class="rounded-lg border bg-card p-6">
							...
							<HomebrewConsumableForm bind:consumable />
						</div>
					</div>
				</div>
			</div>
