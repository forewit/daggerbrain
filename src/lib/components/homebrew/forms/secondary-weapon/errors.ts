...
export function summarizeSecondaryWeaponFormErrors(summary: HomebrewErrorSummary): string[] {
...
function formatWeaponErrorPath(path: HomebrewErrorPath): string {
	if (path.length === 0) return 'Secondary weapon';
	...
	return formatFieldPath(path) || 'Secondary weapon';
}
