...
export function summarizeAncestryCardFormErrors(summary: HomebrewErrorSummary): string[] {
	return summary.fieldErrors.map(
		(error) => `${formatAncestryCardErrorPath(error.path)}: ${error.message}`
	);
}

function formatAncestryCardErrorPath(path: HomebrewErrorPath): string {
	if (path.length === 0) return 'Ancestry card';
	...
	return formatFieldPath(path) || 'Ancestry card';
}
