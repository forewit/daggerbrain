...
export function summarizeTransformationCardFormErrors(summary: HomebrewErrorSummary): string[] {
	return summary.fieldErrors.map(
		(error) => `${formatTransformationCardErrorPath(error.path)}: ${error.message}`
	);
}

function formatTransformationCardErrorPath(path: HomebrewErrorPath): string {
	if (path.length === 0) return 'Transformation card';
	...
	return formatFieldPath(path) || 'Transformation card';
}
