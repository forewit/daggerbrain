<script lang="ts">
	...
	function addConnection() {
		if (character) {
			character.connection_questions.push({ question: '', answer: '' });
			editingIndex = character.connection_questions.length - 1;
		}
	}

	function removeConnection(index: number) {
		if (character) {
			character.connection_questions.splice(index, 1);
	...
	function resetConnections() {
		if (character && primary_class) {
			character.connection_questions = primary_class.connection_questions.map((question) => ({
				question,
				answer: ''
			}));
			editingIndex = null;
