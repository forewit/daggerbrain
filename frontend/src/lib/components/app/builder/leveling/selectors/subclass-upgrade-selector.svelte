<script lang="ts">
  ...
  function get_card_to_show(): Card<"subclass_specialization"> | Card<"subclass_mastery"> | null {
    ...
    if (options_used["tier_3_subclass_upgrade"] >= 1 && options_used["tier_4_subclass_upgrade"] >= 1) {
      ...
      if (
        (option_id === "tier_4_subclass_upgrade" && tier_3_subclass_upgrade_level < tier_4_subclass_upgrade_level) ||
        (option_id === "tier_3_subclass_upgrade" && tier_4_subclass_upgrade_level < tier_3_subclass_upgrade_level)
      ) {
...
<div class="flex flex-col gap-2 bg-primary/50 p-2 rounded-md" bind:clientWidth={width}>
  ...
  >
    <Select.Trigger
      ...
      highlighted={selected_upgrade === null}
    >
