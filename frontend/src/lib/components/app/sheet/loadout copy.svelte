...
{#if character}
  <div class="w-full">
    <!-- loadout -->
    <div class="relative mb-6 mt-3 w-full">
      ...
      {#if loadout.length > 0}
        <CardCarousel cards={loadout} bind:selectedIndex={selectedLoadoutIndex} cardWidth={200} />
      {/if}
      <Button
    ...
    <div class="relative mb-4 mt-3 w-full">
      ...
      {#if vault.length > 0}
        <CardCarousel cards={vault} bind:selectedIndex={selectedVaultIndex} cardWidth={200} />
      {/if}
      {#if restMode}
        <Button
