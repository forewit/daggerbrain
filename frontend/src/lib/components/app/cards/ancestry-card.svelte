...
<div
  style="zoom: {zoom};"
  class={cn(
    "bg-white overflow-hidden flex flex-col relative border-2 border-accent rounded-[20px]",
    size === "small" ? "w-[360px] h-auto" : "w-[360px] h-[504px]",
    className
  )}
>
  <div class="relative flex-1 min-h-0 flex flex-col w-full overflow-y-auto overflow-x-hidden">
    <!-- image and divider -->
    <div
      class={cn("relative h-[105px] max-h-[55%] bg-background", hideImage && "h-auto")}
    >
      {#if !hideImage}
      ...
      />
      <p
        class="absolute -bottom-[25px] right-4 z-50 text-[12px] uppercase text-black tracking-[2px] font-bold"
      >
        ancestry
      </p>
    </div>
    ...
    <div
      ...
      class={cn(
        "text-black flex flex-col justify-start relative overflow-y-auto px-[11px] z-40 pt-1"
      )}
    >
      <div class="flex flex-col items-start">
        <p class="w-full max-w-[71%] text-2xl font-eveleth pb-1 pl-1.5 pt-[6px] uppercase">
          {card.title}
        </p>
  ...
  <div
    class={cn(
      "bg-white flex shrink-0 pt-1 pb-2 items-end justify-between data-[align-right=true]:justify-end px-3 w-full bottom-0 z-40 text-[10px] sm:text-xs",
  ...
  >
    <div class="flex items-center flex-1">
      ...
      />
      <p class="text-black text-[10px] italic w-full relative top-[2px]">
        {card.artist_name}
      </p>
    </div>
    <div class="flex items-end gap-1 relative bottom-0">
      <p class="text-[8px] text-muted-foreground italic flex flex-1 relative top-[1px]">
        Daggerheart™ Compatible. Terms at Daggerheart.com
      </p>
      <img 
        alt="CGL Logo"
        class="w-5 h-5"
        src="/images/logos/cgl-logo.svg"
      />
    </div>
  </div>
</div>
