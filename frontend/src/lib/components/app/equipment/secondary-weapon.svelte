    ...
    {#if showDamageTypeSelect}
      <Select.Root
        ...
        onValueChange={(value) => {
          ...
          character.inventory.secondary_weapons[weapon.id].choices["damage_type"] = value === "" ? [] : [value];
        }}
      >
        <Select.Trigger
    ...
    {#if showTraitSelect}
      <Select.Root
        ...
        onValueChange={(value) => {
          ...
          character.inventory.secondary_weapons[weapon.id].choices["trait"] = value === "" ? [] : [value];
        }}
      >
        <Select.Trigger
          class="text-xs bg-primary-muted rounded-full px-2 py-1 h-auto min-h-0 border-0 shadow-none hover:bg-primary-muted/80 w-auto [&_svg]:size-3 [&_svg]:opacity-50"
