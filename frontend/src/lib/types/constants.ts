import type { BackgroundQuestionAnswer, Character, CharacterInventory, CharacterSettings, ConnectionAnswer, DerivedDescriptors, DomainCardId, LevelUpChoices, LevelUpDomainCardIds } from '$lib/types/character-types';
...
export const CHARACTER_DEFAULTS = {
	...
	settings: JSON.stringify(<CharacterSettings>{
		void_enabled: false,
		use_gold_coins: false
	}),

	// derived
	derived_descriptors: JSON.stringify(<DerivedDescriptors>{
		ancestry_name: "",
		primary_class_name: "",
	...
	experiences: JSON.stringify(<string[]>[ "", ""]),
	...
	background_question_answers: JSON.stringify(<BackgroundQuestionAnswer[]>[ ]),
	connection_answers: JSON.stringify(<ConnectionAnswer[]>[ ]),

	// equipment
	active_armor_id: null,
	active_primary_weapon_id: null,
	active_secondary_weapon_id: null,
	inventory: JSON.stringify(<CharacterInventory>{
		primary_weapons: {},
		secondary_weapons: {},
		armor: {},
		loot: {},
	...
	additional_domain_card_ids: JSON.stringify(<string[]>[ ]),
	additional_character_modifiers: JSON.stringify(<CharacterModifier[]>[ ]),
	additional_weapon_modifiers: JSON.stringify(<WeaponModifier[]>[ ]),

	// ephemeral stats set by the player
	domain_card_choices: JSON.stringify(<Record<string, Record<string, string[]>>>{}),
	domain_card_tokens: JSON.stringify(<Record<string, string[]>>{}),
	selected_traits: JSON.stringify(<Traits>{
		agility: null,
		strength: null,
		finesse: null,
		instinct: null,
	...
	marked_hope: 0,
	marked_armor: 0,
	loadout_domain_card_ids: JSON.stringify(<DomainCardId[]>[ ]),

	// Level up choices
	level: 1,
	level_up_domain_card_ids: JSON.stringify(<LevelUpDomainCardIds>{
		1: { A: null, B: null },
		2: { A: null },
		3: { A: null },
		4: { A: null },
	...
	level_up_choices: JSON.stringify(<LevelUpChoices>{
